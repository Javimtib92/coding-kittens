...
subtitle: Recur... qué?
shortDescription: >
  Explora un enfocament distint per caminar a través d'un objecte utilitzant iteració en lloc de recursió per evitar l'error de límit de la pila.
---

import Highlight from '~/components/ui/Highlight.astro';

Un dia a la feina vaig haver de recórrer una estructura de dades complexa per eliminar algunes referències circulars (Contentful CMS i linked entries, vos sona?) i em vaig trobar amb aquest error:
...
Estava utilitzant una funció recursiva fantàstica per recórrer aquestes dades i de sobte em vaig donar de cara amb la realitat. Vaig ser copetjat per l'error "Maximum call stack exceeded". "Mala tarde". El pitjor de tot és qué aquest error va comprometre produció i, com no estavem manetjant l'error els nostres usuaris estaven experimentant el que s'anomena en inglés la "Black Screen of Death".
...
function normalizeObject(object) {
  if (Array.isArray(object)) {
    ...
    for (const property in object) {
      // Aqui podem fer comprovacions per veure quines propietats son candidats a referències circulars i les podem eliminar o modificar. Com per exemple:
...
3. En cas contrari, si no és `null` i és de tipus `Object`, vol dir que és un objecte. &mdash;array i null són de tipus `Object` a Javascript, per això comprovem que no sigui null&mdash; En aquest cas, itera les seves propietats utilitzant un bucle `for...in`. Per a cada propietat, realitza l'operació desitjada i crida recursivament a `normalizeObject` al valor de la propietat.

Aquest enfocament va funcionar bé fins que no va funcionar.

## Ja cal que et calcis, que la cosa es posa interessant.

Després d'algunes investigacions, vaig trobar una opció de node v8 per incrementar el tamany de la pila anomenada `--stack-size`
però ajustar aquesta opció no em va semblar bé. No tinc res en contra d'aquesta opció, si resol
problema hauria d'estar bé, no es així? Però aquesta opció no és fiable, tal com es diu a [aquesta incidència de Github](https://github.com/nodejs/node/issues/6360).
...
Ei, això sembla prometedor... però segons després em va donar un cop la realitat un altre cop. 🤯
...
2. Inicialitza una matriu que farà de cua i que conté un únic element que serà l'objecte d'entrada,

3. El codi entra en un bucle `while` que continua mentres la cua no estigui buida.
...
són massa complexos i la pila de cridades de la recursivitat creix massa. Hem vist i analitzat un exemple real de una funció recursiva que pot causar aquest error. Després hem vist com s'analitza un problema d'aquest tipus i quines barreres ens hem anat trobant, desde descobrir "Tail Call Optimization" i donar-mos conta que no tots els motors de navegadors ho soporten fins a descobrir que podiem substituir la recursivitat amb un equivalent iteratiu que finalment solucionava el nostre problema.
